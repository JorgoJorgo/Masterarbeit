...
def one_experiment(g, seed, out, algo, f_list, first_string):
    ...
    for f_num in f_list:
        ...
        if num_experiments == 0: #hier tritt ein Fehler auf, wenn das erste Experiment eines Graphen scheitert und es bricht ab
            num_experiments = samplesize #deswegen "überspringe" ich diesen Graphen dann dieses Experiment damit

        if num_experiments < min(len(g.nodes())-1,samplesize):
            ###########################################################DEBUG
            print("num_experiments < min(len(g.nodes())-1,samplesize)")
            print(num_experiments, "< min(" ,len(g.nodes())-1,"," , samplesize , ")") 
            pos = nx.spring_layout(g, seed=225)  
            nx.draw(g, pos, with_labels=True, edge_color='black')
            nx.draw_networkx_edges(g, pos, edgelist=g.graph['fails'], edge_color='red')
            plt.show()
            ###########################################################DEBUG
...
if __name__ == "__main__":
    ...
    attack = 'CLUSTER' #how edge failures are chosen
